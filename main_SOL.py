# -*- coding: utf-8 -*-
"""
Coinalyze APIからSOLのOIデータを取得・加工し、価格データと組み合わせてグラフを生成、
...
PRICE_SYMBOL = "SOLUSDT.6" # BybitのSOL価格データを取得
...
# 画像ファイルのパス
FIGURE_PATH = os.path.join(DATA_DIR, 'sol_oi_analysis_figure.png')

# 対象取引所の設定
EXCHANGE_CONFIG = {
    'Binance': {'code': 'A', 'contracts': ['SOLUSD_PERP.', 'SOLUSDT_PERP.', 'SOLUSD.', 'SOLUSDT.']},
    'Bybit': {'code': '6', 'contracts': ['SOLUSD.', 'SOLUSDT.']},
    'OKX': {'code': '3', 'contracts': ['SOLUSD_PERP.', 'SOLUSDT_PERP.', 'SOLUSD.', 'SOLUSDT.']},
    'BitMEX': {'code': '0', 'contracts': ['SOLUSD_PERP.', 'SOLUSDT_PERP.', 'SOLUSD.', 'SOLUSDT.']}
}
...
def plot_figure(df: pd.DataFrame, save_path: str):
    ...
    # グラフ全体のタイトルに日時を設定
    main_title = f"SOL Open Interest Analysis ({latest_datetime.strftime('%Y-%m-%d %H:%M %Z')})"
    fig.suptitle(main_title, fontsize=16)

    # 1. 価格グラフ (SOLは価格帯が違うため k USD 単位は外す)
